...
class LISNN(nn.Module):
    def __init__(self, opt):
        ...
        if opt.if_lateral:
            self.lateral1 = nn.Conv2d(self.cnn[0][1], self.cnn[0][1], kernel_size = self.li[0], stride = self.li[1], padding = self.li[2], groups = self.kernel[0], bias = False)
            self.lateral2 = nn.Conv2d(self.cnn[1][1], self.cnn[1][1], kernel_size = self.li[0], stride = self.li[1], padding = self.li[2], groups = self.kernel[1], bias = False)
            self.lateral3 = nn.Conv2d(self.cnn[2][1], self.cnn[2][1], kernel_size = self.li[0], stride = self.li[1], padding = self.li[2], groups = self.kernel[2], bias = False)
            self.lateral4 = nn.Conv2d(self.cnn[3][1], self.cnn[3][1], kernel_size = self.li[0], stride = self.li[1], padding = self.li[2], groups = self.kernel[3], bias = False)
            self.lateral5 = nn.Conv2d(self.cnn[4][1], self.cnn[4][1], kernel_size = self.li[0], stride = self.li[1], padding = self.li[2], groups = self.kernel[4], bias = False)
        else:
            self.lateral1 = None
            self.lateral2 = None
            self.lateral3 = None
